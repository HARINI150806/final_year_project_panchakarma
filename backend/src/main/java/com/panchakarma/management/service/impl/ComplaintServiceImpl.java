...
public class ComplaintServiceImpl implements ComplaintService {
    ...
    @Override
    @Transactional
    public List<ComplaintResponse> getComplaintsByPatientUserId(Long identifier) {
        if (identifier == null) {
        ...
        if (userOpt.isPresent()) {
            patient = patientRepository.findByUser(userOpt.get()).orElse(null);
        }
        if (patient == null) {
            patient = patientRepository.findById(identifier).orElse(null);
        }
        if (patient == null) {
            return new ArrayList<>();
        }
    ...
    private ComplaintResponse mapToComplaintResponse(Complaint complaint) {
        ...
        if (complaint.getPatient() != null) {
            Patient p = complaint.getPatient();
            response.setPatientId(p.getUser() != null ? p.getUser().getId() : p.getId());
            String fName = p.getFirstName() != null ? p.getFirstName().trim() : "";
            String lName = p.getLastName() != null ? p.getLastName().trim() : "";
            String fullName = (fName + " " + lName).trim();
            if (fullName.isEmpty() && p.getUser() != null && p.getUser().getFullName() != null) {
                fullName = p.getUser().getFullName();
            }
            response.setPatientName(!fullName.isEmpty() ? fullName : "Patient #" + p.getId());
            response.setPatientEmail(p.getEmail() != null ? p.getEmail() : (p.getUser() != null ? p.getUser().getEmail() : ""));
        }
