...
public class AdminController {
    ...
    public ResponseEntity<List<com.panchakarma.management.dto.PharmacistSummaryResponse>> listPharmacists() {
        return ResponseEntity.ok(adminService.listPharmacists());
    }

    @PostMapping("/pharmacists")
    public ResponseEntity<com.panchakarma.management.dto.PharmacistSummaryResponse> createPharmacist(@Valid @RequestBody com.panchakarma.management.dto.AdminPharmacistRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(adminService.createPharmacist(request));
    }

    @org.springframework.web.bind.annotation.DeleteMapping("/therapists/{id}")
    public ResponseEntity<Void> deleteTherapist(@org.springframework.web.bind.annotation.PathVariable Long id) {
        adminService.deleteUser(id);
        return ResponseEntity.noContent().build();
    }

    @org.springframework.web.bind.annotation.DeleteMapping("/patients/{id}")
    public ResponseEntity<Void> deletePatient(@org.springframework.web.bind.annotation.PathVariable Long id) {
        adminService.deleteUser(id);
        return ResponseEntity.noContent().build();
    }

    @org.springframework.web.bind.annotation.DeleteMapping("/pharmacists/{id}")
    public ResponseEntity<Void> deletePharmacist(@org.springframework.web.bind.annotation.PathVariable Long id) {
        adminService.deleteUser(id);
        return ResponseEntity.noContent().build();
    }
