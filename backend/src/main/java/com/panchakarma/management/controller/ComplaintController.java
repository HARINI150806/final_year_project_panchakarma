...
public class ComplaintController {
    ...
    @GetMapping({"/therapist/patients/{userId}/complaints", "/therapists/patients/{userId}/complaints"})
    @PreAuthorize("hasAnyAuthority('ROLE_THERAPIST', 'ROLE_ADMIN', 'THERAPIST', 'ADMIN')")
    public ResponseEntity<List<ComplaintResponse>> getComplaintsByPatientUserId(@PathVariable Long userId) {
        List<ComplaintResponse> complaints = complaintService.getComplaintsByPatientUserId(userId);
        return ResponseEntity.ok(complaints);
    }

    @GetMapping({"/therapist/complaints", "/therapists/complaints"})
    @PreAuthorize("hasAnyAuthority('ROLE_THERAPIST', 'ROLE_ADMIN', 'THERAPIST', 'ADMIN')")
    public ResponseEntity<List<ComplaintResponse>> getTherapistComplaints() {
        List<ComplaintResponse> complaints = complaintService.getTherapistComplaints();
        return ResponseEntity.ok(complaints);
    }
}
